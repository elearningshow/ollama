...
func TestMultiModelStress(t *testing.T) {
	...
	for i := 0; i < len(req); i++ {
		...
		if i > 1 && consumed > vram {
			slog.Info("achieved target vram exhaustion", "count", i, "vramformat.MebiByte", vram/format.MebiByte, "modelsMB", consumed/format.MebiByte)
			break
		}
		consumed += chosenModels[i].size
		slog.Info("target vram", "count", i, "vramformat.MebiByte", vram/format.MebiByte, "modelsMB", consumed/format.MebiByte)
